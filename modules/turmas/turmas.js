(function(){
  ...
  function renderCard(c){
    ...
    return `<article class="turma-list-row">
      ...
      <span>${esc(teacherLabel(c.teacherId))}</span>
      <span>${esc(bookLabel(c.bookId))}</span>
      <span>${fmtNumber(students.length)} aluno(s)</span>
      <span>${summary.present}/${summary.total} presentes${meeting?` · ${fmtDate(meeting.date)}`:''}</span>
      <div class="turma-row-actions">
        <button class="btn primary small" onclick="PurpleTurmas.open('${esc(c.id)}')">Abrir aula</button>
        ${user().role!=='teacher'?`<button class="btn ghost small" onclick="event.stopPropagation();App.editClass('${esc(c.id)}')">Editar</button><button class="btn soft small" onclick="event.stopPropagation();PurpleTurmas.completeClass('${esc(c.id)}')">Concluir</button><button class="btn ghost small" onclick="event.stopPropagation();PurpleTurmas.archiveClass('${esc(c.id)}')">Arquivar</button><button class="btn danger small" onclick="event.stopPropagation();PurpleTurmas.deleteClass('${esc(c.id)}')">Excluir</button>`:''}
      </div>
    </article>`;
  }
  ...
  function renderModule(mode='classes'){
    ...
    return `<div class="page turmas-page">
      <div class="section-title"><div><span class="eyebrow">Centro operacional da aula</span><h2>${user().role==='teacher'?'Minhas turmas':'Turmas'}</h2><p>Chamada, execução da aula e acompanhamento no mesmo fluxo.</p></div><div class="section-actions">${user().role!=='teacher'?`<button class="btn ghost" onclick="PurpleTurmas.showCalendar()">Feriados e recessos</button><button class="btn ghost" onclick="PurpleTurmas.openInbox()">Acompanhamentos ${inboxCount?`(${inboxCount})`:''}</button><button class="btn soft" onclick="App.editClass()">Nova turma</button>`:''}</div></div>
      ...
      <section class="turma-list-table"><div class="turma-list-head"><span>Turma</span><span>Professor</span><span>Livro</span><span>Alunos</span><span>Chamada</span><span>Ações</span></div>${classes.map(renderCard).join('')||'<div class="empty"><h3>Nenhuma turma autorizada</h3><p>Verifique professor vinculado, permissão ou cadastro acadêmico.</p></div>'}</section>
    </div>`;
  }

  function renderTurmasNav(active='classes'){
    return `<nav class="turmas-module-tabs"><button class="${active==='classes'?'active':''}" onclick="PurpleTurmas.showClasses()">Turmas</button><button class="${active==='templates'?'active':''}" onclick="PurpleTurmas.showTemplates()">Schedules padrão</button><button class="${active==='calendar'?'active':''}" onclick="PurpleTurmas.showCalendar()">Feriados e recessos</button></nav>`;
  }
  ...
  function renderClass(c){
    ...
    return `<div class="page turma-workspace">
      <div class="turma-topline"><button class="btn ghost small" onclick="PurpleTurmas.back()">Voltar</button><div><span class="turma-book-status ${norm(book).includes('NAO VINCULADO')?'warning':''}">${esc(book)}</span><h2>${esc(c.name)}</h2><p><b>${esc(teacher)}</b><span>${esc(blocks.map(b=>[b.day,b.time].filter(Boolean).join(' · ')).join(' / ')||'Horário não informado')}</span></p></div></div>
  ...
  function renderAttendanceRow(c,row,meeting=null){
    const buttons=[['present','Presente'],['absent','Falta'],['justified','Falta justificada'],['replacement','Reposição realizada']];
    const meetingId=meeting?.id||'';
    const rowId=`att-${c.id}-${meetingId||'main'}-${row.student.id}`;
    return `<article class="attendance-row ${row.status}">
      <div class="attendance-mainline"><button class="student-link" onclick="PurpleTurmas.quickStudent('${esc(row.student.id)}','${esc(c.id)}')">${esc(row.student.name)}</button><div class="attendance-actions">${buttons.map(([status,label])=>`<button class="${row.status===status?'active':''}" onclick="const wasActive=this.classList.contains('active');if(wasActive)this.classList.remove('active');PurpleTurmas.setAttendance('${esc(c.id)}','${esc(row.student.id)}','${status}','${esc(meetingId)}',{clear:wasActive})">${label}</button>`).join('')}</div></div>
      <span class="save-state" id="${esc(rowId)}">${row.updatedAt?'Salvo':'--'}</span>
    </article>`;
  }

  function renderStudents(c){
    return `<section class="panel"><div class="panel-head"><div><h3>Alunos da turma</h3><small>Nome clicável para consulta operacional rápida.</small></div><div class="section-actions"><button class="btn primary small" onclick="PurpleTurmas.openAddStudent('${esc(c.id)}')">Adicionar aluno</button></div></div><div class="turma-student-list">${classStudents(c.id).map(s=>{const stats=attendanceStats(s.id,c.id),freq=stats?.percent??(s.frequency||0);return `<article><button onclick="PurpleTurmas.quickStudent('${esc(s.id)}','${esc(c.id)}')"><b>${esc(s.name)}</b><span>${esc(s.level||c.level||'Nível não informado')} · freq. ${fmtNumber(freq)}%${stats?` · ${fmtNumber(stats.absences)} faltas · ${fmtNumber(stats.replacements)} reposições`:''}</span></button><div class="table-actions"><button class="btn ghost small" onclick="PurpleTurmas.signal('${esc(s.id)}','${esc(c.id)}')">Registrar acompanhamento</button><button class="btn danger small" onclick="PurpleTurmas.unlinkStudent('${esc(c.id)}','${esc(s.id)}')">Remover da turma</button></div></article>`}).join('')||'<div class="empty"><p>Nenhum aluno vinculado.</p></div>'}</div></section>`;
  }
  function renderSchedule(c){
    ...
    if(schedule){
      ...
      return `<section class="panel turma-schedule-premium"><div class="panel-head"><div><span class="eyebrow">${esc(schedule.templateTitle)}</span><h3>Cronograma real da turma</h3><small>${fmtNumber(progress.done)} / ${fmtNumber(progress.planned)} horas-aula realizadas · conclusão prevista ${fmtDate(schedule.projectedEndDate)}</small></div><div class="section-actions"><button class="btn ghost small" onclick="PurpleTurmas.insertLesson('${esc(c.id)}')">Inserir aula</button><button class="btn ghost small" onclick="PurpleTurmas.previewSchedule('${esc(c.id)}')">Regerar preview</button></div></div><section class="student-attendance-summary"><span><b>Planejado</b>${fmtNumber(progress.planned)} HA</span><span><b>Realizado</b>${fmtNumber(progress.done)} HA</span><span><b>Pendentes</b>${fmtNumber(progress.pending)}</span><span><b>${fmtNumber(progress.percent)}%</b> progresso</span></section>${selected?renderMeetingClassroom(c,selected):''}<div class="schedule-timeline">${meetings.map(meeting=>`<article class="schedule-node ${meeting.id===selected?.id?'active':''} ${meeting.localChange?'local-change':''} ${meeting.status==='cancelled'?'cancelled':''} ${meetingDone(c,meeting)?'done':''}" role="button" tabindex="0" onclick="PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')}"><time>${fmtDate(meeting.date)}<small>${esc(meeting.time||'--')}</small></time><div><div class="schedule-node-head"><h4>Encontro ${String(meeting.encounterOrder).padStart(2,'0')}</h4><span>${meetingStatus(c,meeting)}</span></div>${meeting.localChange?'<small class="local-badge">Ajuste desta turma</small>':'<small class="local-badge base">Conforme plano base</small>'}${meeting.blocks.map(block=>`<p><b>${String(block.order).padStart(2,'0')} · ${esc(block.title)}</b><small>${esc(block.unit||'')} ${esc(block.bookPages?`p. ${block.bookPages}`:'')} · ${esc(block.content)}</small></p>`).join('')}<div class="table-actions"><button class="btn primary small" onclick="event.stopPropagation();PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')">Abrir aula</button><button class="btn success small" onclick="event.stopPropagation();PurpleTurmas.completePlanned('${esc(c.id)}','${esc(meeting.id)}')">${meetingDone(c,meeting)?'Confirmada':'Aula dada conforme cronograma'}</button><button class="btn ghost small" onclick="event.stopPropagation();PurpleTurmas.registerDivergence('${esc(c.id)}','${esc(meeting.id)}')">Registrar divergência</button><button class="btn danger small" onclick="event.stopPropagation();PurpleTurmas.cancelMeeting('${esc(c.id)}','${esc(meeting.id)}')">Cancelar / não realizada</button></div></div></article>`).join('')}</div></section>`;
    }
    return `<section class="panel"><div class="panel-head"><div><h3>Cronograma</h3><small>Grade oficial preservada do cadastro de Turmas. Gere o cronograma real a partir de um cronograma base.</small></div><button class="btn primary small" onclick="PurpleTurmas.previewSchedule('${esc(c.id)}')">Gerar cronograma</button></div><div class="table-wrap"><table><thead><tr><th>Dia</th><th>Horário</th><th>Sala</th></tr></thead><tbody>${classBlocks(c).map(b=>`<tr><td>${esc(b.day||'--')}</td><td>${esc(b.time||'--')}</td><td>${esc(b.room||c.room||'--')}</td></tr>`).join('')||'<tr><td colspan="3">Sem grade cadastrada.</td></tr>'}</tbody></table></div></section>`;
  }
  function renderMeetingClassroom(c,meeting){
    ...
    return `<section class="panel meeting-classroom-panel ${done?'done':''}"><div class="panel-head"><div><span class="eyebrow">Aula selecionada</span><h3>Encontro ${String(meeting.encounterOrder).padStart(2,'0')} · ${fmtDate(meeting.date)} · ${esc(meeting.time||'Horário a confirmar')}</h3><small>Presença e falta ficam vinculadas a esta data e a este encontro.</small></div><div class="section-actions"><button class="btn ${done?'success':'primary'} small" onclick="PurpleTurmas.completePlanned('${esc(c.id)}','${esc(meeting.id)}')">${done?'Aula confirmada':'Aula dada conforme cronograma'}</button><button class="btn soft small" onclick="PurpleTurmas.allPresent('${esc(c.id)}','${esc(meeting.id)}')">Todos presentes</button><button class="btn ghost small" onclick="PurpleTurmas.noteClass('${esc(c.id)}','${esc(meeting.id)}')">Registrar informações</button></div></div><div class="meeting-classroom-grid"><div class="meeting-content-pane"><div class="pane-kicker">Conteúdo do encontro</div><div class="turma-lesson-focus">${(meeting.blocks||[]).map(block=>`<div class="today-block"><div><strong>Bloco ${block.order} · ${esc(block.title)}</strong><span>${esc(block.content)}</span></div><small>${esc(block.type||'Aula')} · Páginas: ${esc(block.bookPages||'--')}</small></div>`).join('')||'<p class="muted">Nenhum conteúdo vinculado a este encontro.</p>'}</div>${note?.text?`<div class="alert green"><div class="alert-icon">✓</div><div><b>Registro da aula</b><span>${esc(note.text)}</span></div></div>`:''}${note?.divergence?`<div class="alert yellow"><div class="alert-icon">!</div><div><b>Divergência registrada</b><span>${esc(note.divergence)}</span></div></div>`:''}</div><aside class="meeting-students-pane"><div class="meeting-students-head"><div><span class="eyebrow">Frequência</span><h3>Alunos desta aula</h3><small>${summary.present} presentes · ${summary.absent} faltas · ${summary.justified} justificadas · ${summary.replacement} reposições · ${summary.pending} pendentes</small></div></div><div class="attendance-list">${attendanceRows(c,meeting).map(row=>renderAttendanceRow(c,row,meeting)).join('')||'<div class="empty"><p>Nenhum aluno vinculado a esta turma.</p></div>'}</div></aside></div></section>`;
  }
  ...
  function renderClassGrades(c){
    const rows=classGradeRows(c),students=classStudents(c.id),summaries=students.map(student=>({student,grades:studentClassGradeSummary(student.id,c.id),freq:attendanceStats(student.id,c.id)}));
    ...
    return `<section class="panel"><div class="panel-head"><div><h3>Notas da turma</h3><small>Média automática de 0 a 100. Avalia participação, skills, homework, Midterm Exam e Final Term.</small></div><div class="section-actions"><button class="btn primary small" onclick="PurpleTurmas.openClassGrade('${esc(c.id)}')">Incluir nota</button></div></div><section class="student-attendance-summary"><span><b>Alunos</b>${fmtNumber(students.length)}</span><span><b>Lançamentos</b>${fmtNumber(rows.length)}</span><span><b>Média geral</b>${avg===null?'--':`${fmtNumber(avg,1)}%`}</span><span><b>Regra</b>70%</span></section><div class="table-wrap"><table><thead><tr><th>Aluno</th><th>Participation</th><th>Speaking</th><th>Listening</th><th>Writing</th><th>Reading</th><th>Midterm</th><th>Final Term</th><th>Homework</th><th>Média</th><th>Frequência</th><th>Situação</th><th>Ações</th></tr></thead><tbody>${summaries.map(({student,grades,freq})=>`<tr><td><button class="student-link" onclick="PurpleTurmas.quickStudent('${esc(student.id)}','${esc(c.id)}')">${esc(student.name||'--')}</button></td>${grades.weighted.map(item=>`<td>${item.score===null?'--':`<button class="student-link" onclick="PurpleTurmas.openClassGrade('${esc(c.id)}','${esc((grades.rows.find(row=>norm(row.type)===norm(item.type))||{}).id||'')}')">${fmtNumber(item.score,1)}</button>`}</td>`).join('')}<td>${grades.homeworkAverage===null?'--':fmtNumber(grades.homeworkAverage,1)}</td><td><b>${grades.finalAverage===null?'--':`${fmtNumber(grades.finalAverage,1)}%`}</b></td><td>${fmtNumber(freq.percent,1)}%</td><td><span class="status ${grades.status==='Aprovado'&&freq.percent>=70?'approved':grades.status==='Aguardando'?'draft':'rejected'}">${grades.status==='Aguardando'?'Aguardando':(grades.finalAverage>=70&&freq.percent>=70?'Aprovado':'Reprovado')}</span></td><td><button class="btn primary small" onclick="PurpleTurmas.openClassGrade('${esc(c.id)}','','${esc(student.id)}')">Lançar</button></td></tr>`).join('')||'<tr><td colspan="13">Nenhum aluno vinculado para lançar notas.</td></tr>'}</tbody></table></div><details class="class-grade-details" open><summary>Lançamentos detalhados para editar</summary><div class="table-wrap"><table><thead><tr><th>Aluno</th><th>Unidade</th><th>Tipo</th><th>Nota</th><th>Data</th><th>Situação</th><th>Ações</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${esc(row.studentName||'--')}</td><td>${esc(row.unit||'--')}</td><td>${esc(row.type||'--')}</td><td>${Number.isFinite(Number(row.score))?fmtNumber(row.score,1):'--'}</td><td>${fmtDate(row.date)}</td><td>${esc(row.status||'Lançado')}</td><td><div class="table-actions"><button class="btn ghost small" onclick="PurpleTurmas.openClassGrade('${esc(c.id)}','${esc(row.id)}')">Editar</button><button class="btn danger small" onclick="PurpleTurmas.deleteClassGrade('${esc(c.id)}','${esc(row.id)}')">Excluir</button></div></td></tr>`).join('')||'<tr><td colspan="7">Nenhuma nota lançada.</td></tr>'}</tbody></table></div></details></section>`;
  }
  ...
  async function cancelMeeting(classId,meetingId){
    const schedule=classGeneratedSchedule(classId),meeting=schedule?.meetings.find(item=>item.id===meetingId);if(!meeting||!confirm('Marcar aula como não realizada e reprogramar conteúdo?'))return;
    meeting.status='cancelled';meeting.localChange=true;meeting.changeReason='Aula não realizada';
    const nextEncounter=Math.max(0,...schedule.meetings.map(item=>Number(item.encounterOrder)||0))+1,maxOrder=Math.max(0,...schedule.meetings.flatMap(item=>(item.blocks||[]).map(block=>Number(block.order)||0)));
    const blocks=(meeting.blocks||[]).map((block,index)=>({...block,id:uid('reprogrammed-block'),order:maxOrder+index+1,encounterOrder:nextEncounter,localChange:true,notes:'Reprogramado após aula não realizada'}));
    const clone={id:uid('reprogrammed-meeting'),encounterOrder:nextEncounter,date:schedule.projectedEndDate||meeting.date,time:meeting.time,status:'planned',blockIds:blocks.map(block=>block.id),blocks,localChange:true,changeReason:`Conteúdo reprogramado do encontro ${meeting.encounterOrder}`};
  ...
  async function saveClassGrade(classId,gradeId=''){
    ...
    const scoreRaw=$('#classGradeScore')?.value,score=scoreRaw===''?null:Number(scoreRaw);
