(function(){
  ...
  function renderHistory(c){
    const schedule=classGeneratedSchedule(c.id),meetings=scheduleMeetings(schedule),students=classStudents(c.id),events=[];
    const studentName=id=>students.find(student=>student.id===id)?.name||(db().students||[]).find(student=>student.id===id)?.name||'Aluno';
    const meetingLabel=id=>meetings.find(meeting=>meeting.id===id);
    (ensure().signals||[]).filter(s=>s.classId===c.id).forEach(s=>events.push({date:s.updatedAt||s.createdAt||s.date,type:'Sinal',title:`${s.studentName||'Aluno'} · ${s.type||'Acompanhamento'}`,text:s.description||'',meta:`${fmtDate(s.date)} · ${s.priority||'Rotina'} · ${(s.destinations||[]).join(' + ')}`}));
    Object.entries(ensure().meetingNotes||{}).filter(([key])=>key.startsWith(`${c.id}::`)).forEach(([key,note])=>{const meetingId=key.split('::')[1],meeting=meetingLabel(meetingId);if(note?.text)events.push({date:note.updatedAt||meeting?.date,type:'Aula',title:`Registro da aula · Encontro ${String(meeting?.encounterOrder||'').padStart(2,'0')}`,text:note.text,meta:`${fmtDate(meeting?.date)} · ${note.by||'Equipe'}`});if(note?.divergence)events.push({date:note.updatedAt||meeting?.date,type:'Divergência',title:`Divergência · Encontro ${String(meeting?.encounterOrder||'').padStart(2,'0')}`,text:note.divergence,meta:`${fmtDate(meeting?.date)} · ${note.by||'Equipe'}`})});
    Object.entries(ensure().attendance||{}).filter(([key])=>key.startsWith(`${c.id}::`)).forEach(([key,map])=>{const [,date,meetingId]=key.split('::'),rows=Object.entries(map||{});if(rows.length)events.push({date:rows.map(([,row])=>row.updatedAt).filter(Boolean).sort().at(-1)||date,type:'Frequência',title:`Chamada lançada · ${fmtDate(date)}`,text:rows.map(([studentId,row])=>`${studentName(studentId)}: ${row.status}`).join(' · '),meta:`${rows.length} registro(s)${meetingId&&meetingId!=='main'?` · ${meetingLabel(meetingId)?.encounterOrder?`Encontro ${meetingLabel(meetingId).encounterOrder}`:''}`:''}`})});
    classGradeRows(c).forEach(row=>events.push({date:row.updatedAt||row.date,type:'Nota',title:`${row.studentName||'Aluno'} · ${row.type||'Nota'}`,text:`${row.unit||'Geral'} · ${Number.isFinite(Number(row.score))?fmtNumber(row.score,1):'--'}%`,meta:`${fmtDate(row.date)} · ${row.status||'Lançado'}`}));
    Object.entries(ensure().blockExecution||{}).filter(([key])=>key.startsWith(`${c.id}::`)).forEach(([key,row])=>{const blockId=key.split('::')[1],meeting=meetings.find(item=>(item.blocks||[]).some(block=>block.id===blockId)),block=(meeting?.blocks||[]).find(item=>item.id===blockId);events.push({date:row.updatedAt||meeting?.date,type:'Conteúdo',title:`${block?.title||'Bloco'} · ${row.status||'Atualizado'}`,text:`Bloco ${block?.order||'--'}${block?.bookPages?` · páginas ${block.bookPages}`:''}`,meta:`${fmtDate(meeting?.date)} · ${row.by||'Equipe'}`})});
    meetings.filter(meeting=>meeting.localChange||meeting.changeReason).forEach(meeting=>events.push({date:meeting.updatedAt||meeting.date,type:'Cronograma',title:`Encontro ${String(meeting.encounterOrder).padStart(2,'0')} · ${meeting.changeReason||'Ajuste local'}`,text:(meeting.blocks||[]).map(block=>block.title).join(' · '),meta:`${fmtDate(meeting.date)} · ${meeting.status||'planejado'}`}));
    events.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
    return `<section class="panel"><div class="panel-head"><div><h3>Histórico da turma</h3><small>Movimentações do cronograma, chamadas, notas, sinais e registros de aula.</small></div></div><div class="timeline">${events.map(item=>`<div class="timeline-item"><b>${esc(item.type)} · ${esc(item.title)}</b><p>${esc(item.text||'Atualização registrada no sistema.')}</p><time>${esc(item.meta||fmtDate(String(item.date||'').slice(0,10)))}</time></div>`).join('')||'<div class="empty"><p>Nenhuma movimentação registrada.</p></div>'}</div></section>`;
  }
  ...
  async function postponeMeeting(classId,meetingId){
    return moveMeetingDate(classId,meetingId,{confirmFirst:true,message:'Aula movida e cronograma recalculado.'});
  }
  ...
  function openHomeworkGrades(classId,studentId){
    const c=(db().classes||[]).find(item=>item.id===classId),student=(db().students||[]).find(item=>item.id===studentId);
    if(!c||!student)return toast('Turma ou aluno não encontrado.');
    const rows=classGradeRows(c).filter(row=>row.studentId===studentId&&norm(row.type).includes('HOMEWORK'));
    const avg=rows.length?rows.reduce((sum,row)=>sum+Number(row.score||0),0)/rows.length:null;
    showModal(`<div class="modal-head"><div><span class="eyebrow">Homework</span><h3>${esc(student.name)}</h3><p class="helper">${rows.length?`${fmtNumber(rows.length)} lançamentos · média ${fmtNumber(avg,1)}%`:'Nenhum homework lançado ainda.'}</p></div><button class="modal-close" onclick="App.closeModal()">×</button></div><div class="class-grade-entry-list homework-modal-list">${rows.map(row=>`<article><div><b>${esc(row.unit||'Geral')}</b><span>${Number.isFinite(Number(row.score))?fmtNumber(row.score,1):'--'}% · ${esc(row.status||'Lançado')}</span><small>${fmtDate(row.date)}${row.notes?` · ${esc(row.notes)}`:''}</small></div><div class="table-actions"><button class="btn ghost small" onclick="PurpleTurmas.openClassGrade('${esc(classId)}','${esc(row.id)}')">Editar</button><button class="btn danger small" onclick="PurpleTurmas.deleteClassGrade('${esc(classId)}','${esc(row.id)}')">Excluir</button></div></article>`).join('')||'<div class="empty"><p>Nenhum homework lançado para este aluno.</p></div>'}</div><div class="section-actions"><button class="btn ghost" onclick="App.closeModal()">Fechar</button><button class="btn primary" onclick="PurpleTurmas.openClassGrade('${esc(classId)}','','${esc(studentId)}')">Lançar homework</button></div>`);
  }
