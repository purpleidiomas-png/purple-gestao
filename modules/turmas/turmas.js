(function(){
  ...
  function renderSchedule(c){
    ...
    if(schedule){
      ...
      return `<section class="panel turma-schedule-premium"><div class="schedule-compact-head"><div><span class="eyebrow">${esc(schedule.templateTitle)}</span><h3>Cronograma</h3><p><b>${fmtNumber(progress.done)} de ${fmtNumber(progress.planned)} HA</b> realizadas · ${fmtNumber(progress.percent)}% · conclusão prevista ${fmtDate(schedule.projectedEndDate)}</p><div class="schedule-progress"><span style="width:${Math.min(100,Math.max(0,Number(progress.percent)||0))}%"></span></div></div><div class="section-actions"><button class="btn primary small" onclick="PurpleTurmas.insertLesson('${esc(c.id)}')">Inserir aula</button><details class="action-menu" onclick="event.stopPropagation()"><summary aria-label="Ações do cronograma">•••</summary><div><button onclick="PurpleTurmas.previewSchedule('${esc(c.id)}')">Regerar preview</button></div></details></div></div>${selected?renderMeetingClassroom(c,selected):''}<div class="schedule-timeline compact">${meetings.map(meeting=>{const done=meetingDone(c,meeting),status=meetingStatus(c,meeting),firstBlock=(meeting.blocks||[])[0]||{};return `<article class="schedule-node ${meeting.id===selected?.id?'active':''} ${meeting.localChange?'local-change':''} ${meeting.status==='cancelled'?'cancelled':''} ${done?'done':''}" role="button" tabindex="0" onclick="PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')}"><time><b>${fmtDate(meeting.date)}</b><small>${esc(meeting.time||'--')}</small></time><div><div class="schedule-node-head"><h4><i aria-hidden="true"></i> Encontro ${String(meeting.encounterOrder).padStart(2,'0')}</h4><span>${esc(status)}</span></div><p><b>${esc(firstBlock.unit||'Unit')}</b><small>${(meeting.blocks||[]).map(block=>`${esc(block.title)}${block.bookPages?` · p. ${esc(block.bookPages)}`:''}`).join(' / ')}</small></p></div><div class="timeline-actions"><button class="btn primary small" onclick="event.stopPropagation();PurpleTurmas.openMeeting('${esc(c.id)}','${esc(meeting.id)}')">Abrir</button><details class="action-menu" onclick="event.stopPropagation()"><summary aria-label="Ações do encontro">•••</summary><div><button onclick="PurpleTurmas.completePlanned('${esc(c.id)}','${esc(meeting.id)}')">${done?'Desmarcar confirmação':'Confirmar aula'}</button><button onclick="PurpleTurmas.registerDivergence('${esc(c.id)}','${esc(meeting.id)}')">Registrar divergência</button><button class="danger" onclick="PurpleTurmas.cancelMeeting('${esc(c.id)}','${esc(meeting.id)}')">Cancelar / não realizada</button></div></details></div></article>`}).join('')}</div></section>`;
    }
