...
const yl4Template = api.scheduleTemplate('young-learners-4-oficial-v1');
...
assert(yl4Template, 'template YOUNG LEARNERS 4 oficial carrega');
...
assert.strictEqual(api.scheduleTemplates().length, 13, 'modelos oficiais ficam disponíveis');
...
assert.strictEqual(yl4Template.blocks.length, 46, 'Young Learners 4 preserva 46 horas-aula');
assert.strictEqual(api.groupedTemplateBlocks(yl4Template).length, 23, 'Young Learners 4 preserva 23 encontros');
assert.strictEqual(yl4Template.blocks[2].bookPages, '5-16', 'Young Learners 4 preserva paginas do bloco 3');
assert.strictEqual(yl4Template.blocks[41].unit, 'Unit 14', 'Young Learners 4 preserva Unit 14');
assert.strictEqual(yl4Template.blocks[44].title, 'FINAL TEST', 'Young Learners 4 preserva Final Test');
assert.strictEqual(yl4Template.blocks[45].type, 'Final Exam', 'Young Learners 4 preserva tipo Final Exam');
assert(!yl4Template.blocks.some(block => /2026|abr|mai|jun|jul|ago|set|out|nov|dez/i.test(`${block.bookPages} ${block.content} ${block.title}`)), 'Young Learners 4 nao depende das datas do PDF');
